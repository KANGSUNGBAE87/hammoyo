...
- 2026-06-24 보정: Supabase/권한/응답/AI/copy 정책, 디자인 보강 기획, 이미지 생성문, 정적 MVP 샘플, ko/en preview 전환, localStorage-only 경계가 현재 프로젝트에 추가되었습니다.
- 2026-06-25 구현: `SupabaseBackendAdapter`, Supabase Edge Function runtime connection code, DeepSeek V4 Pro AI provider proxy, AI copy policy, `verify:backend-ai` 검증을 추가했습니다. 원격 DB 적용, Function 배포, secret 주입, live 호출 검증은 아직 하지 않았습니다.
...
- 다음 필요 실행: 정적 MVP 기준 확인 후 실제 구현 착수 지시, 또는 방장/총무/운영진 8~12명 모집 경로 확보 후 live validation 재개. 조건부 P4 패키지는 생성됐지만 실제 응답은 0건이며 `FINAL_APPROVED`는 아닙니다.
...
1. 약속방 생성 및 preview 전용 초대 문구 확인
...
- `docs/mvp/index.html` — 디자인 보강 기획 기반 정적 MVP 샘플. 실제 초대 링크 공유, Toss login, Supabase 저장은 아직 연결하지 않음.
...
- `supabase/migrations/20260624_hammoyo_backend.sql` — SQL Editor 검토용 `hammoyo_` prefixed Supabase/RLS schema 초안. 원격 적용은 아직 하지 않음.
...
- Design sample: `docs/mvp/index.html`을 브라우저로 열어 화면 상태를 확인합니다. `?lang=en`으로 영어 preview를 열 수 있습니다.
