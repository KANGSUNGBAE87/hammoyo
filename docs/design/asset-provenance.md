...
> A cute group of friendly animal characters gathering to plan a meetup around a calendar and clock, soft modern mobile app illustration, no text, no logo, no existing brand or character reference.

## 개별 캐릭터 Crops

아래 파일은 메인 이미지에서 재사용을 위해 잘라 저장한 asset입니다.

- `docs/assets/characters/hammoyo-character-cat.png`
- `docs/assets/characters/hammoyo-character-rabbit.png`
...
> A soft mobile app background wallpaper with cute 3D clay-like animal friends planning a meetup, spacious pale background, no text, no logo, no brand references.

## 저작권 / 출시 메모

- 외부 IP, 실존 브랜드, 기존 캐릭터명, 로고, 카카오톡/Toss UI를 prompt에 넣지 않았습니다.
- 이 asset은 함모여 앱용 신규 생성 이미지로 관리합니다.
- Apps in Toss 제출 전에는 실제 스토어/콘솔 정책에 맞춰 AI 생성 이미지 사용 고지 필요 여부를 한 번 더 확인합니다.
