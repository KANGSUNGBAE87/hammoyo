...
- `docs/final-delivery/reference/contact-sheets/screen_samples_contact_sheet.png`
- `docs/final-delivery/reference/contact-sheets/split_assets_contact_sheet.png`

## 2026-06-27 재정렬 결정

- 첫 화면은 final delivery의 귀여운 3D 동물 hero 중심으로 정리합니다.
- 기능 화면은 장식보다 카드, chip, progress, 추천 근거가 먼저 보이게 유지합니다.
...
| Brand | `#6B5CFF` | 주요 CTA, 추천 1순위, 선택 상태 |
| Brand strong | `#5A46F2` | CTA active/gradient |
| Brand soft | `#EDEBFF` | 추천 카드 배경, 정보 영역 |
| Success | `#22C55E` | 가능/완료 |
| Warning | `#F59E0B` | 조정 가능, 응답 부족 |
| Danger | `#EF4444` | 어려움, 만료·오류 보조 표시 |
| Border | `#E5E8F1` | 구분선 |
...
| `SCR-03-private-negotiation` | 비공개 조정 | 한 명의 확인/조정 상태를 개인 지목 없이 표시 |
| `SCR-04-result-recommendation` | 추천 결과 | fallback 추천 1~2개, 근거, 공유 문구 |
| `SCR-05-insufficient-response` | 응답 부족 | 아직 추천하지 않고 기다릴 이유 표시 |
| `SCR-06-link-expired` | 만료 링크 | 읽기/쓰기 제한 안내 |
| `SCR-07-room-closed` | 확정 완료 | 재계산 금지, 확정 문구 고정 |
